<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <h2 class="text-xl font-bold leading-tight text-gray-900">Editar producto</h2>
                <p class="text-sm text-gray-500">{{ $platillo->nombre }}</p>
            </div>
    ...
    <div class="py-8">
        <div class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
            <form action="{{ route('cocina.platillos.update', $platillo) }}" method="POST" class="space-y-5 rounded-md border border-gray-200 bg-white p-6 shadow-sm">
                @csrf
                @method('PUT')

                <div>
                    <x-input-label for="nombre" value="Nombre" />
                    <x-text-input id="nombre" name="nombre" type="text" class="mt-1 block w-full" :value="old('nombre', $platillo->nombre)" required autofocus />
                ...
                <div>
                    ...
                    <textarea id="descripcion" name="descripcion" rows="4" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('descripcion', $platillo->descripcion) }}</textarea>
                    <x-input-error :messages="$errors->get('descripcion')" class="mt-2" />
                </div>

                <div>
                    <x-input-label for="precio" value="Precio" />
                    <x-text-input id="precio" name="precio" type="number" min="0" step="0.01" class="mt-1 block w-full" :value="old('precio', $platillo->precio)" required />
                    <x-input-error :messages="$errors->get('precio')" class="mt-2" />
                </div>

                <div>
                    <x-input-label for="disponible" value="Disponibilidad" />
                    <select id="disponible" name="disponible" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
