        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @forelse($ordenes as $orden)
                    <div class="bg-white rounded-xl shadow-sm border-l-8 overflow-hidden flex flex-col justify-between h-full min-h-[240px] transition transform hover:-translate-y-1 hover:shadow-md
                    ...
                    ">
                        ...
                        <div class="p-4 bg-gray-50 border-b border-gray-100 flex justify-between items-center">
                            <div>
                                <span class="block text-lg font-extrabold text-gray-800">Pedido #{{ str_pad($orden->id, 3, '0', STR_PAD_LEFT) }}</span>
                                <span class="text-[11px] text-gray-400 font-medium">Recibido: {{ $orden->created_at->format('H:i A') }}</span>
                            </div>
                        ...
                        <div class="p-4 flex-grow flex flex-col justify-between">
                            <div class="mb-3 flex items-center justify-between">
                                <span class="text-xs uppercase font-bold tracking-wider text-gray-400">Estado actual:</span>
                                ...
                                @if($orden->detallesOrden && $orden->detallesOrden->count() > 0)
                                    @foreach($orden->detallesOrden as $detalle)
                                        <p class="text-sm font-semibold text-gray-700 flex justify-between">
                                            <span>• {{ $detalle->cantidad }}x {{ $detalle->platillo ? $detalle->platillo->nombre : 'Platillo #' . $detalle->platillo_id }}</span>
                                        </p>
                                    @endforeach
                                @else
                                    <p class="text-xs text-gray-400 italic">Esta orden no tiene productos registrados.</p>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="p-3 bg-gray-50 border-t border-gray-100 flex flex-col space-y-2">
                            <form action="{{ route('cocina.marcarLista', $orden->id) }}" method="POST" class="w-full">
                                @csrf
                                <button type="submit" class="w-full text-center bg-zinc-800 hover:bg-zinc-900 text-white text-xs font-bold py-2 px-4 rounded-lg shadow uppercase tracking-wider transition">
                ...
                @empty
                    <div class="col-span-1 md:col-span-2 lg:col-span-3 bg-white p-8 rounded-xl text-center italic text-gray-500 border border-dashed border-gray-300 font-medium">
